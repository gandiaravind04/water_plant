  ...
  const fetchTransactions = async () => {
    try {
      ...
      const r = await axios.get(
        import.meta.env.VITE_API_URL + "/api/cans",
        authCfg(),
      );
  ...
  const calculateStats = (txs) => {
    ...
    txs.forEach((tx) => {
      if (tx.status !== "returned") {
        ...
        activeCount++;
        if (tx.status === "overdue" || new Date() > new Date(tx.dueAt))
          overdueCans += rem;
      }
    });
    ...
    <div className="page-container">
      ...
      <div className="dashboard-grid">
        ...
        <div className="glass-panel metric-card overdue">
          ...
          <h2 style={{ fontSize: "32px", color: "var(--status-overdue)" }}>
            {stats.overdueCans}
          </h2>
          <p
            style={{
        ...
        <div className="glass-panel metric-card">
          ...
          <h2 style={{ fontSize: "32px", color: "var(--status-pending)" }}>
            ₹{stats.amountDue}
          </h2>
          <p
            style={{
          ...
          >
            {active.map((tx) => {
              ...
              return (
                <div
                  key={tx._id}
                  className={`glass-panel ${isOverdue ? "overdue-pulse" : ""}`}
                  style={{
                    padding: "20px",
                    borderRadius: "14px",
                    borderLeft: `4px solid ${isOverdue ? "var(--status-overdue)" : "var(--status-pending)"}`,
                  }}
                >
                  <div
                    style={{
                  ...
                  >
                    <div>
                      ...
                      >
                        <strong className="glow-text-cyan">
                          {tx.village?.name || "—"}
                        </strong>
                      </p>
                    </div>
                  ...
                  >
                    <div style={{ fontSize: "14px" }}>
                      ...
                      <strong style={{ fontSize: "16px" }}>{rem}</strong>/
                      {tx.cansIssued}
                    </div>
                    <div style={{ fontSize: "14px" }}>
                      <span style={{ color: "var(--text-muted)" }}>
